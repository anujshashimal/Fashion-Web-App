...
export class Addcategory extends Component {
    ...
    componentDidUpdate() {
        axios.get('http://100.24.72.11:5000/category/findCategory/'+this.state.MainCategory+'/'+this.state.Admin)
            .then(response=>{
        ...
        if(this.state.CategoryName != "" ){
            if(this.state.MainCategory !=""){
                if(this.state.Admin != ""){
                    ...
                    axios.get('http://100.24.72.11:5000/category/findCategorybyname/'+this.state.MainCategory+'/'+this.state.CategoryName)
                        .then(response=> {
                            if (response.data.length == 0) {

                                axios.get('http://100.24.72.11:5000/categoryID/CategoryID')
                                    .then(response=>{
                                        if(response.data.length>0){
                                            this.setState({
                                        ...
                                        axios.post('http://100.24.72.11:5000/category/addCategory',Category)
                                            .then(res=>console.log(res.data));
                                        const CategoryID ={
                                            "Category_ID":this.state.CategoryID+1
                                        }
                                        axios.post('http://100.24.72.11:5000/categoryID/updateCategoryID',CategoryID)
                                            .then(res=>console.log(res.data));
                                        this.setState({
                                            CategoryID: "",
                                            CategoryName: "",
